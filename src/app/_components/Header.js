...
export default function Header({ dict, lang }) {
  ...
  // Garantir que lang tenha um valor padrão para os links
    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-100">
      <div className="container mx-auto px-4 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={`/${currentLang}/carrinho`} className="relative p-2 text-gray-600 hover:text-green-700 transition">
      ...
      {/* Menu Mobile Simples caso o arquivo MobileMenu.js não esteja pronto */}
      {isMenuOpen && (
        <div className="md:hidden bg-white border-b p-4 flex flex-col gap-4">
          <Link href={`/${currentLang}/produtos`} onClick={() => setIsMenuOpen(false)}>Produtos</Link>
          <Link href={`/${currentLang}/sobre`} onClick={() => setIsMenuOpen(false)}>Sobre</Link>
          <Link href={`/${currentLang}/contato`} onClick={() => setIsMenuOpen(false)}>Contato</Link>
        </div>
      )}
    </header>
  );
